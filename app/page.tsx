...
export async function generateMetadata({
  ...
  let description = "The internet's best collection of funny videos, dank photos, dark humor, and viral trends. Updated every single day.";
  ...
  return {
    title,
    description,
    keywords: ["memes", "funny videos", "dank memes", "viral", "relatable", q].filter(Boolean),
    openGraph: {
      title,
      description,
      type: "website",
  ...
  if (memes.length === 0) {
    return (
      <div className="text-center py-20 animate-in fade-in duration-500">
        ...
        <p className="text-gray-500 font-medium mt-2">Try searching for something else or clear the search.</p>
      </div>
    );
  }

  return <MemeGrid key={`${type}-${q}`} initialMemes={memes} type={type} searchQuery={q} />;
}
    ...
    <div className="min-h-screen bg-[#F4F4F5] text-gray-900 selection:bg-purple-300 selection:text-white pb-10 overflow-x-hidden">
      ...
      <header className="fixed top-0 left-0 right-0 z-50 bg-white/70 backdrop-blur-xl backdrop-saturate-150 border-b border-gray-200/50 shadow-sm transition-all">
        <div className="max-w-7xl mx-auto flex justify-between items-center px-6 py-4">
          ...
          <nav className="hidden md:flex gap-8 font-bold text-gray-500">
            <Link href="/" className="hover:text-black transition-colors">Trending</Link>
            <Link href="/" className="hover:text-black transition-colors">Fresh</Link>
            <Link href="/" className="hover:text-black transition-colors">Categories</Link>
          </nav>

          <Link href="/upload" className="bg-black text-white px-6 py-2.5 rounded-xl font-bold hover:bg-purple-600 hover:scale-105 active:scale-95 transition-all shadow-md flex items-center gap-2">
            <span>📤</span> <span className="hidden sm:inline">Submit</span>
          </Link>
        </div>
      </header>
      ...
      <section className="max-w-4xl mx-auto px-6 py-10 text-center flex flex-col items-center animate-in slide-in-from-bottom-4 fade-in duration-700 ease-out">
        <h1 className="text-5xl md:text-7xl font-black mb-6 tracking-tight text-gray-900 leading-tight">
          Unlimited Laughs.<br/>
          <span className="text-transparent bg-clip-text bg-linear-to-r from-purple-600 to-black">Daily Memes.</span>
        </h1>
        <h2 className="text-lg text-gray-500 font-medium max-w-2xl mb-10">
          The internet's best collection of funny videos, dark humor, and viral trends. Updated every single day.
        </h2>
        ...
        <form action="/" className="w-full max-w-2xl mb-10 relative group z-10">
          ...
          />
          <span className="absolute left-5 top-1/2 -translate-y-1/2 text-2xl text-gray-400 select-none">🔍</span>
          ...
          <button type="submit" className="absolute right-3 top-1/2 -translate-y-1/2 bg-black text-white px-6 py-3 rounded-xl font-bold hover:bg-purple-600 active:scale-95 transition-all">
            Search
          </button>
        </form>
        ...
        <div className="bg-white p-1.5 rounded-2xl shadow-sm border border-gray-100 flex gap-1 z-0 relative">
          <Link
            ...
            className={`px-6 sm:px-8 py-3 rounded-xl font-bold text-sm transition-all duration-300 ${
              type === "all" ? "bg-black text-white shadow-md scale-100" : "text-gray-500 hover:text-black hover:bg-gray-50 active:scale-95"
            }`}
          >
            🔥 All
          </Link>
          <Link
            ...
            className={`px-6 sm:px-8 py-3 rounded-xl font-bold text-sm transition-all duration-300 ${
              type === "video" ? "bg-black text-white shadow-md scale-100" : "text-gray-500 hover:text-black hover:bg-gray-50 active:scale-95"
            }`}
          >
            🎥 Videos
          </Link>
          <Link
            ...
            className={`px-6 sm:px-8 py-3 rounded-xl font-bold text-sm transition-all duration-300 ${
              type === "image" ? "bg-black text-white shadow-md scale-100" : "text-gray-500 hover:text-black hover:bg-gray-50 active:scale-95"
            }`}
          >
            📸 Photos
          </Link>
      ...
      <footer className="max-w-7xl mx-auto px-6 py-10 mt-20 border-t border-gray-200 flex flex-col sm:flex-row justify-between items-center gap-4">
        ...
        <div className="flex gap-6 font-bold text-sm text-gray-500">
          ...
          <Link href="/contact" className="hover:text-black transition-colors">Contact & Feedback</Link>
        </div>
      </footer>
